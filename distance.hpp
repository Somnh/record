...
inline float IPSQ8_ext(const float *x, const uint8_t *y, int d, const float *mi,
  ...
  for (int i = 0; i < d; i += 4) {
    // Load 4 uint8_t values from y
    uint8x8_t zz_u8 = vld1_u8(y + i);
    // Convert uint8_t to uint16_t (to avoid overflow) and then to float32
    uint16x8_t zz_u16 = vmovl_u8(zz_u8);  // Widen uint8 to uint16
    float32x4_t yy = vcvtq_f32_u32(vmovl_u16(vget_low_u16(zz_u16)));  // Convert uint16 to float

    // Add 0.5f to the y values
    yy = vaddq_f32(yy, dot5);

    // Load mi and dif values
    float32x4_t mi_vec = vld1q_f32(mi + i);
    float32x4_t dif_vec = vld1q_f32(dif + i);

    // Perform the yy * dif + mi * 255 operation
    yy = vmulq_f32(yy, dif_vec);
    yy = vmlaq_f32(yy, mi_vec, const_255);

    // Load x values
    float32x4_t x_vec = vld1q_f32(x + i);

    // Multiply x by yy and accumulate to sum
    sum = vfmaq_f32(sum, x_vec, yy);
  }
#else
  float sum = 0.0;
  for (int i = 0; i < d; ++i) {
